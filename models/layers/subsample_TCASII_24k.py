...
def OneStream_block_wise_Predict(xyz, npoint, blockNum, scale, bias):
    ...
    # pass the sparsed xyz into fps get the sparsed output
    spar_out = my_fps(xyz_0_temp, int(npoint/scale)) #(64,256) *2有点问题 #FIXME
    # classfy the output into 8 domain and count it number
    B, _ = spar_out.size()
    spar_out = spar_out.to(torch.int64)
    ...
    xyz_0_temp = query < checkpoint_0
    xyz_0_xless05 = torch.abs(query) < checkpoint_1
    if blockNum == 2:
...
class FurthestPointSampling(Function):
    ...
    def forward(ctx, xyz: torch.Tensor, npoint: int) -> torch.Tensor:
        ...
        if npoint== 6000 or npoint==11838 :
            output = MultiSteram_blockwise_fps_test(xyz, npoint, blockNum, scale, PMS, BMS)# actually spar 16 in algrithom 
        elif npoint == 1500 or npoint==2959:
            output = MultiSteram_blockwise_fps_test(xyz, npoint, 8, 8, 1, 1)
            # output = fps_myown_seg(xyz, 8, npoint)
        elif npoint == 375 or npoint==739:
            output = MultiSteram_blockwise_fps_test(xyz, npoint, 4, 4, 1, 1)
